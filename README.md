...
-   **Гибкая фильтрация**: Позволяет выборочно рендерить приложения (например, только ветку `master`) без обработки всего списка.
...
-   `--filter`: Условие для выборки приложений (см. ниже).
-   `--log-level` (`-l`): Уровень логирования (`debug`, `info`, `warn`, `error`). По умолчанию `warn`.

#### Фильтрация (--filter)

Позволяет рендерить только те приложения, которые соответствуют условию.
Синтаксис: `путь.к.полю=значение` или `путь.к.полю!=значение`.
...
> **Важно**: Фильтрация поддерживает проход по вложенным полям (Maps), но **не поддерживает** списки (Arrays/Lists).
...
2.  **Парсинг и Фильтрация**: Утилита читает YAML-вывод и находит все ресурсы с `kind: Application`.
    *   Если задан флаг `--filter`, происходит проверка указанного поля в YAML-структуре.
    *   Приложения, не соответствующие фильтру, игнорируются на этом этапе (репозитории для них не клонируются).
